...
# class ActorMannNaive(nn.Module):
...
#             for i, layer in enuemrate(net):
#                 if isinstance(layer, nn.Linear):
#                     weight = layer.weight
#                     bias = layer.bias

#             self.final[i].weight = coefs * weights
#             self.final[i].bias = coefs * biases
...
class ActorCriticNetMann(nn.Module):
    ...
    def sample_best_actions(self, inputs):
        ...
        # out = 0
        # for i in range(8):
        #     out += output_list[i] * gating_weights[i]
        return out
